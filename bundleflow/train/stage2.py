...
from bundleflow.models.menu import MenuElement, make_null_element, revenue_loss, utilities_matrix
from bundleflow.data import load_cats_dir, train_test_split, gen_uniform_iid_xor
from bundleflow.valuation.valuation import XORValuation, _tensor_to_mask
...
# ---------- μのウォームスタート ----------
@torch.no_grad()
def warmstart_mus(flow: BundleFlow, menu: List[MenuElement], t_grid: torch.Tensor, 
...
# ---------- 弱い再初期化 ----------
@torch.no_grad()
def reinit_unused_elements(flow: BundleFlow, menu: List[MenuElement], Z: torch.Tensor,
...
def train_stage2(args):
    ...
    # チェックポイントと同じ設定でモデル作成
    print(f"[Stage2] Creating BundleFlow with m={ckpt['m']}, use_spectral_norm={use_spectral_norm}", flush=True)
    flow = BundleFlow(m=ckpt["m"], use_spectral_norm=use_spectral_norm).to(device)
    print(f"[Stage2] Loading model state_dict...", flush=True)
    flow.load_state_dict(ckpt["model"])
    print(f"[Stage2] Freezing BundleFlow parameters...", flush=True)
...
@torch.no_grad()
def eval_hard_revenue(flow: BundleFlow, V: List[XORValuation], menu: List[MenuElement], t_grid: torch.Tensor) -> float:
    device = t_grid.device
    total = 0.0
    for v in V:
